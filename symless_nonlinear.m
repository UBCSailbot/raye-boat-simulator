...
v_tw = 1;     % speed of wind in n-frame
alpha_tw = 1; % angle of wind in n-frame

% Note: n-frame is xyz North-East-Down
%       b-frame is xyz Forward-Right-Down

%% define state space
x = 1;    % m, position in n-frame
y = 1;    % m, position in n-frame
phi = 1;  % rad, angle around x-axis n-frame
psi = 1;  % rad, angle around z-axis n-frame (note: z is pointed down)
...
surge=1; % m/s, linear velocity forward in b-frame (same as u in paper)
sway=1;  % m/s, linear velocity right in b-frame (same as v in paper)
roll=1;  % rad/s, angular velocity around x-axis in b-frame (same as p in paper)
yaw=1;   % rad/s, angular velocity around z-axis in b-frame (same as r in paper)
