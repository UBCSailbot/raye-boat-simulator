...
v_tw = 20; %True wind velocity, m/s
alpha_tw = 3*pi/4; %True wind angle (n-frame), radians
...
sangle_in = pi/6; %Starting Sail position, radians
rangle_in = 0; %Starting rudder angle, radians

sangle_time = timeseries(sangle_in,linspace(1,0.1,55));
rangle_time = timeseries(rangle_in,linspace(1,0.1,55));
