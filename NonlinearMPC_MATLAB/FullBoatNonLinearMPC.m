...
nlobj.PredictionHorizon = 3; % Prediction horizon is the number of time steps a control action is analysed over
nlobj.ControlHorizon = 2; % Control Horizon is [1, Prediction Horizon]
=--------------------------
nlobj.Model.StateFcn = "fullBoat_DT_RK23"; % give the ODE system of our boat to the Nonlinear MPC
...
EKF = extendedKalmanFilter(@fullBoat_DT_RK23, @boatMeasurementFcn);
...
for ct = 1:(Duration/Ts)
    count1 = count1+1
     xk = correct(EKF, y);
     [mv,nloptions,info] = nlmpcmove(nlobj,x,mv,y_ref,[5, pi/6]);
     predict(EKF, [mv ; Ts]);
      x = fullBoat_DT_RK23(x,mv);
      y = x(4) + randn*0.01;
